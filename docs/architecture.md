...
flowchart TD
    ...
    A --> T["graph_tools.py: node_info, neighbors, trace_money,<br/>common_receivers, top_nodes, cluster_info, simulate_removal"]
    ...
    T --> MCP["MCP-сервер graph-intel: 7 tools,<br/>2 resources, 1 prompt"]
    AI --> U
```
...
Проверка качества (в `run_meta.json`): случайная отложенная выборка 25% — ROC-AUC 0.71 против 0.66 у бейзлайна «число плательщиков»; искусственная граница (обучение на коленах 1–2, прогноз колена 3, 789 узлов, доля пересылающих 0.36) — ROC-AUC 0.60 против 0.55. Перенос между коленами слабый, поэтому порог консервативный: terminal только при `P < 0.35` и заметной сумме, остальные узлы 4-го колена помечены как неопределённые. Параметры стандартизации и intercept сохранены в `run_meta.json`, модель воспроизводима.
...
**MCP-сервер (`mcp_server.py`, FastMCP, mcp<2)**: tools `node_info, neighbors, trace_money, common_receivers, top_nodes, cluster_info, simulate_removal`; resources `graph://methodology`, `graph://node/{gid}`; prompt `investigate`. Транспорт stdio или Streamable HTTP (`--http`, порт 8010).
...
Опциональные пункты: учёт обрыва графа (модель + проверка на границе), временные паттерны (быстрый транзит, синхронные поступления), циклы возврата, устойчивость сети (стресс-тест), AI-ассистент аналитика, автогенерация карточки узла, оценка полноты (рекомендация запросить выписку для 4-го колена).
