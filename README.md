...
**Требования:** Python 3.10–3.12, ~300 МБ диска, интернет не нужен (нужен только для опционального LLM).
...
Windows: `run.bat`, Linux/macOS: `bash run.sh`. Оба скрипта ставят зависимости, прогоняют пайплайн и открывают UI.
...
1. **Обрыв на 4-м колене.** Проверено на данных: ребро всегда записано на колене *(глубина плательщика + 1)*. Значит,
   ...
   - коэффициенты модели — в `run_meta.json` и во вкладке «Методика».
...
- Модель для 4-го колена переносит поведение колен 1–3 на 4-е; это допущение, поэтому такие узлы помечены отдельно.
